...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await GarudaReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.garuda.address, '100', '1000', { from: owner });

        await this.garuda.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setGarudaReferral(this.referral.address, { from: owner });
